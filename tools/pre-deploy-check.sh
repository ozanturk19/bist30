...
# 2. Python compile
echo ""
echo "2/74 Python compile (app.py)..."
...
# tutulmalı.
echo ""
echo "3/74 KALICI_KURALLAR audit..."
...
# 4. format-lint
echo ""
echo "4/74 format-lint (CPO-1180 K6)..."
...
echo ""
echo "5/74 CSS token guard (CPO-1349)..."
...
echo "6/74 style-guard (T1.7: sablon ici var()/ham hex/yerel :root/bos catch ratchet)..."
...
echo ""
echo "7/74 lint_scope ratchet (T9.4: sablon sayisi daralma dedektoru)..."
...
echo ""
echo "8/74 node-syntax-check (DEV2-T-MOBOVF-1: sablon-ici JS sozdizimi)..."
...
echo ""
echo "9/74 state-order-check (K-G: durum kurali varyantin ALTINDA olmali)..."
...
echo ""
echo "10/74 contrast-check (K-I: ayni kuralda bg+fg WCAG kontrasti)..."
...
echo ""
echo "11/74 cachebust-check (K-J: ?v= referansi diskteki hash ile ayni mi)..."
...
echo ""
echo "12/74 legal-text-sync-check (K-H: hukuki metin <-> 'Son guncelleme' senkronu)..."
...
echo ""
echo "13/74 da-override-check (K-K: .da-* bildirimini kisayolla silme)..."
...
echo ""
echo "14/74 canon-conflict-check (K-AP: kanonik kaynak <-> sayfa-yerel kopya)..."
...
echo ""
echo "15/74 glyph-canon-check (K-AQ: rozet glifi <-> kanonik anlam)..."
...
echo ""
echo "16/74 threshold-sync-check (K-AR: bant esigi kopyalari <-> business_rules)..."
...
echo ""
echo "17/74 nav-label-canon-check (K-AS: gezinme etiketi kanonu)..."
...
echo ""
echo "18/74 tooltip-canon-check (K-AT: ipucu mekanizmasi kanonu)..."
...
echo ""
echo "19/74 sort-canon-check (K-AV: siralama kontrolu durum kanonu)..."
...
echo ""
echo "20/74 newtab-canon-check (K-AW: yeni sekme uyarisi kanonu)..."
...
echo ""
echo "21/74 autorefresh-canon-check (K-AX: periyodik yenileme odak kanonu)..."
...
echo "22/74 sticky-anchor-check (K-AY: yapiskan capa kanonu)..."
...
echo "23/74 safearea-layer-check (K-AZ: guvenli alan kapsam kanonu)..."
...
echo "24/74 tr-decimal-input-check (K-BA: TR ondalik girdi kanonu)..."
...
echo "25/74 tr-calendar-day-check (K-BD: TR takvim gunu kanonu)..."
...
echo "26/74 global-collision-check (K-BE: kuresel ad cakismasi)..."
...
echo "27/74 js-palette-check (K-BG: JS paleti <-> tokens.css, RATCHET)..."
...
echo "28/74 copy-promise-check (K-BH: \"kopyala\" vaadi = kopyalanan sey)..."
...
echo "29/74 merge-report-check (K-BI: birlestirme sayimi = gercekten eklenen)..."
...
echo "30/74 position-validation-check (K-BJ: pozisyon sayisal dogrulamasi tek kanon)..."
...
echo "31/74 api-data-loading-guard (K-BK: bos liste != veri yok)..."
...
echo "32/74 local-data-guard (K-BM: bozuk yerel kayit sessizce silinmemeli)..."
...
echo "33/74 eod-freshness-claim-check (K-BN: gosterilen fiyatin tazelik vaadi)..."
...
echo "34/74 volume-axis-color-check (K-BO: hacim ekseni yon rengiyle boyanamaz)..."
...
echo "35/74 direction-zero-check (K-BP+K-CC: 0 bir yonle ayni kefeye konamaz)..."
...
echo "36/74 stop-level-canon-check (K-BQ: 'Stop' yon iddiasidir)..."
...
echo "37/74 indicator-precision-check (K-BR: ADX/RSI 1 ondalik kanonu)..."
...
echo "38/74 indicator-panel-canon-check (K-BS: rozet ve teknik satir tek kaynak)..."
...
echo "39/74 innerhtml-sibling-check (K-BT: innerHTML kardes bileseni yok etmemeli)..."
...
echo "40/74 label-derived-number-check (K-BU: sayi etiketten ayristirilmaz)..."
...
echo "41/74 money-scale-canon-check (K-BV: para olcegi kisaltmasi tek kanon)..."
...
echo "42/74 inline-style-hex-check (K-BW: satir-ici renk token'dan gelir)..."
...
echo "43/74 fundamental-card-band-check (K-BX: temel kart birim/band)..."
...
echo "44/74 template-color-channel-check (K-BY: renk kanaldan bagimsiz token'dan gelir)..."
...
echo "45/74 provenance-badge-check (K-BZ: koken rozeti <-> govdenin gercek kaynagi)..."
...
echo "46/74 rr-canon-check (K-CA: cubuk da sayi da rr_signal'den)..."
...
echo "47/74 window-claim-check (K-CB: pencere beyani <-> cizilen veri)..."
...
echo "48/74 export-parity-check (K-CC: disa aktarim <-> ekran ayni sayi/ad)..."
...
echo "49/74 relative-time-anchor-check (K-CD: goreli zaman <-> okuyucunun takvimi)..."
...
echo "50/74 intraday-claim-check (K-CE: EOD-only mimaride gun-ici/canli iddia)..."
...
echo "51/74 blog-claim-check (K-CF: blog govdesi icerik-dogruluk kanonu)..."
...
echo "52/74 app-published-text-check (K-CG: app.py yayimlanan metin kanonu)..."
...
echo "53/74 static-js-text-check (K-CH: paylasilan JS yayimlanan metin kanonu)..."
...
echo "54/74 manifest-text-check (K-CI: PWA manifest yayimlanan metin + URL sozu)..."
...
echo "55/74 crawler-channel-check (K-CJ: robots/llms/humans/security + JSON-LD)..."
...
#      duzeltildi.
echo "56/74 sitemap-claim-check (K-CK: changefreq <-> lastmod <-> EOD mimarisi)..."
...
echo "57/74 head-meta-check (K-CL: <head> meta kanali + og gorseli)..."
...
echo "58/74 glossary-where-check (K-CM: sozlugun \"nerede gorulur\" vaadi)..."
...
echo "59/74 score-claim-check (K-CN: Teknik Güç Skoru bilesen iddiasi)..."
...
echo "60/74 delivery-timing-claim-check (K-CO: teslimat anindaligi vaadi)..."
...
echo "61/74 tab-visibility-canon-check (K-CP: sekme gorunurlugu tek kanon)..."
...
echo "62/74 url-state-param-check (K-CQ: URL durum parametresi korunumu)..."
...
echo "63/74 form-url-state-check (K-CS: form durumu <-> adres cubugu tur-gidisi)..."
...
echo "64/74 url-state-owner-check (K-CT: adres cubugunun tek sahibi)..."
...
echo "65/74 client-server-state-owner-check (K-CU: sunucu durumunun istemcideki sahibi)..."
...
echo "66/74 privacy-claim-flow-check (K-CV: gizlilik beyani <-> veri akisi)..."
...
echo "67/74 score-band-canon-check (K-CW: skor bandinin renk+ad kanonu)..."
...
echo "68/74 dynamic-tip-canon-check (K-CX: durum degisince data-tip de guncellenmeli)..."
...
echo "69/74 canon-css-dup-check (K-CY: kanonik CSS'i JS enjeksiyonu ezemez)..."
...
echo "70/74 brand-wordmark-canon-check (K-CZ: marka sozcuk-isareti tek token)..."
...
echo "71/74 py-palette-check (K-DB: Python kanali <-> tokens.css, SIFIR sarti)..."
...
echo "72/74 index-ticker-channel-check (K-DC: endeks/hisse kanal envanteri)..."
...
echo "73/74 long-only-surface-check (K-DE: long-only sunum kapisi)..."
...
echo "74/74 tr-fold-canon-check (K-DG: Turkce arama katlamasi tek kanon)..."
