...
# 2. Python compile
echo ""
echo "2/14 Python compile (app.py)..."
...
# tutulmalı.
echo ""
echo "3/14 KALICI_KURALLAR audit..."
...
# 4. format-lint
echo ""
echo "4/14 format-lint (CPO-1180 K6)..."
...
echo ""
echo "5/14 CSS token guard (CPO-1349)..."
...
echo "6/14 style-guard (T1.7: sablon ici var()/ham hex/yerel :root/bos catch ratchet)..."
...
echo ""
echo "7/14 lint_scope ratchet (T9.4: sablon sayisi daralma dedektoru)..."
...
echo ""
echo "8/14 node-syntax-check (DEV2-T-MOBOVF-1: sablon-ici JS sozdizimi)..."
...
echo ""
echo "9/14 state-order-check (K-G: durum kurali varyantin ALTINDA olmali)..."
...
echo ""
echo "10/14 contrast-check (K-I: ayni kuralda bg+fg WCAG kontrasti)..."
...
echo ""
echo "11/14 cachebust-check (K-J: ?v= referansi diskteki hash ile ayni mi)..."
...
echo ""
echo "12/14 legal-text-sync-check (K-H: hukuki metin <-> 'Son guncelleme' senkronu)..."
...
echo ""
echo "13/14 da-override-check (K-K: .da-* bildirimini kisayolla silme)..."
...
echo ""
echo "14/14 canon-conflict-check (K-AP: kanonik kaynak <-> sayfa-yerel kopya)..."
