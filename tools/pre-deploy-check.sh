...
# 2. Python compile
echo ""
echo "2/78 Python compile (app.py)..."
...
# tutulmalı.
echo ""
echo "3/78 KALICI_KURALLAR audit..."
...
# 4. format-lint
echo ""
echo "4/78 format-lint (CPO-1180 K6)..."
...
echo ""
echo "5/78 CSS token guard (CPO-1349)..."
...
echo "6/78 style-guard (T1.7: sablon ici var()/ham hex/yerel :root/bos catch ratchet)..."
...
echo ""
echo "7/78 lint_scope ratchet (T9.4: sablon sayisi daralma dedektoru)..."
...
echo ""
echo "8/78 node-syntax-check (DEV2-T-MOBOVF-1: sablon-ici JS sozdizimi)..."
...
echo ""
echo "9/78 state-order-check (K-G: durum kurali varyantin ALTINDA olmali)..."
...
echo ""
echo "10/78 contrast-check (K-I: ayni kuralda bg+fg WCAG kontrasti)..."
...
echo ""
echo "11/78 cachebust-check (K-J: ?v= referansi diskteki hash ile ayni mi)..."
...
echo ""
echo "12/78 legal-text-sync-check (K-H: hukuki metin <-> 'Son guncelleme' senkronu)..."
...
echo ""
echo "13/78 da-override-check (K-K: .da-* bildirimini kisayolla silme)..."
...
echo ""
echo "14/78 canon-conflict-check (K-AP: kanonik kaynak <-> sayfa-yerel kopya)..."
...
echo ""
echo "15/78 glyph-canon-check (K-AQ: rozet glifi <-> kanonik anlam)..."
...
echo ""
echo "16/78 threshold-sync-check (K-AR: bant esigi kopyalari <-> business_rules)..."
...
echo ""
echo "17/78 nav-label-canon-check (K-AS: gezinme etiketi kanonu)..."
...
echo ""
echo "18/78 tooltip-canon-check (K-AT: ipucu mekanizmasi kanonu)..."
...
echo ""
echo "19/78 sort-canon-check (K-AV: siralama kontrolu durum kanonu)..."
...
echo ""
echo "20/78 newtab-canon-check (K-AW: yeni sekme uyarisi kanonu)..."
...
echo ""
echo "21/78 autorefresh-canon-check (K-AX: periyodik yenileme odak kanonu)..."
...
echo "22/78 sticky-anchor-check (K-AY: yapiskan capa kanonu)..."
...
echo "23/78 safearea-layer-check (K-AZ: guvenli alan kapsam kanonu)..."
...
echo "24/78 tr-decimal-input-check (K-BA: TR ondalik girdi kanonu)..."
...
echo "25/78 tr-calendar-day-check (K-BD: TR takvim gunu kanonu)..."
...
echo "26/78 global-collision-check (K-BE: kuresel ad cakismasi)..."
...
echo "27/78 js-palette-check (K-BG: JS paleti <-> tokens.css, RATCHET)..."
...
echo "28/78 copy-promise-check (K-BH: \"kopyala\" vaadi = kopyalanan sey)..."
...
echo "29/78 merge-report-check (K-BI: birlestirme sayimi = gercekten eklenen)..."
...
echo "30/78 position-validation-check (K-BJ: pozisyon sayisal dogrulamasi tek kanon)..."
...
echo "31/78 api-data-loading-guard (K-BK: bos liste != veri yok)..."
...
echo "32/78 local-data-guard (K-BM: bozuk yerel kayit sessizce silinmemeli)..."
...
echo "33/78 eod-freshness-claim-check (K-BN: gosterilen fiyatin tazelik vaadi)..."
...
echo "34/78 volume-axis-color-check (K-BO: hacim ekseni yon rengiyle boyanamaz)..."
...
echo "35/78 direction-zero-check (K-BP+K-CC: 0 bir yonle ayni kefeye konamaz)..."
...
echo "36/78 stop-level-canon-check (K-BQ: 'Stop' yon iddiasidir)..."
...
echo "37/78 indicator-precision-check (K-BR: ADX/RSI 1 ondalik kanonu)..."
...
echo "38/78 indicator-panel-canon-check (K-BS: rozet ve teknik satir tek kaynak)..."
...
echo "39/78 innerhtml-sibling-check (K-BT: innerHTML kardes bileseni yok etmemeli)..."
...
echo "40/78 label-derived-number-check (K-BU: sayi etiketten ayristirilmaz)..."
...
echo "41/78 money-scale-canon-check (K-BV: para olcegi kisaltmasi tek kanon)..."
...
echo "42/78 inline-style-hex-check (K-BW: satir-ici renk token'dan gelir)..."
...
echo "43/78 fundamental-card-band-check (K-BX: temel kart birim/band)..."
...
echo "44/78 template-color-channel-check (K-BY: renk kanaldan bagimsiz token'dan gelir)..."
...
echo "45/78 provenance-badge-check (K-BZ: koken rozeti <-> govdenin gercek kaynagi)..."
...
echo "46/78 rr-canon-check (K-CA: cubuk da sayi da rr_signal'den)..."
...
echo "47/78 window-claim-check (K-CB: pencere beyani <-> cizilen veri)..."
...
echo "48/78 export-parity-check (K-CC: disa aktarim <-> ekran ayni sayi/ad)..."
...
echo "49/78 relative-time-anchor-check (K-CD: goreli zaman <-> okuyucunun takvimi)..."
...
echo "50/78 intraday-claim-check (K-CE: EOD-only mimaride gun-ici/canli iddia)..."
...
echo "51/78 blog-claim-check (K-CF: blog govdesi icerik-dogruluk kanonu)..."
...
echo "52/78 app-published-text-check (K-CG: app.py yayimlanan metin kanonu)..."
...
echo "53/78 static-js-text-check (K-CH: paylasilan JS yayimlanan metin kanonu)..."
...
echo "54/78 manifest-text-check (K-CI: PWA manifest yayimlanan metin + URL sozu)..."
...
echo "55/78 crawler-channel-check (K-CJ: robots/llms/humans/security + JSON-LD)..."
...
#      duzeltildi.
echo "56/78 sitemap-claim-check (K-CK: changefreq <-> lastmod <-> EOD mimarisi)..."
...
echo "57/78 head-meta-check (K-CL: <head> meta kanali + og gorseli)..."
...
echo "58/78 glossary-where-check (K-CM: sozlugun \"nerede gorulur\" vaadi)..."
...
echo "59/78 score-claim-check (K-CN: Teknik Güç Skoru bilesen iddiasi)..."
...
echo "60/78 delivery-timing-claim-check (K-CO: teslimat anindaligi vaadi)..."
...
echo "61/78 tab-visibility-canon-check (K-CP: sekme gorunurlugu tek kanon)..."
...
echo "62/78 url-state-param-check (K-CQ: URL durum parametresi korunumu)..."
...
echo "63/78 form-url-state-check (K-CS: form durumu <-> adres cubugu tur-gidisi)..."
...
echo "64/78 url-state-owner-check (K-CT: adres cubugunun tek sahibi)..."
...
echo "65/78 client-server-state-owner-check (K-CU: sunucu durumunun istemcideki sahibi)..."
...
echo "66/78 privacy-claim-flow-check (K-CV: gizlilik beyani <-> veri akisi)..."
...
echo "67/78 score-band-canon-check (K-CW: skor bandinin renk+ad kanonu)..."
...
echo "68/78 dynamic-tip-canon-check (K-CX: durum degisince data-tip de guncellenmeli)..."
...
echo "69/78 canon-css-dup-check (K-CY: kanonik CSS'i JS enjeksiyonu ezemez)..."
...
echo "70/78 brand-wordmark-canon-check (K-CZ: marka sozcuk-isareti tek token)..."
...
echo "71/78 py-palette-check (K-DB: Python kanali <-> tokens.css, SIFIR sarti)..."
...
echo "72/78 index-ticker-channel-check (K-DC: endeks/hisse kanal envanteri)..."
...
echo "73/78 long-only-surface-check (K-DE: long-only sunum kapisi)..."
...
echo "74/78 tr-fold-canon-check (K-DG: Turkce arama katlamasi tek kanon)..."
...
echo "75/78 learning-mode-surface-check (K-DH: Ogrenme Modu yuzey envanteri)..."
...
echo "76/78 signal-rule-canon-check (K-DK: sinyal kurali tek kanon)..."
...
echo "77/78 fundamental-field-presentation-check (K-DL: isaretli yuzde + kaynaksiz alan)..."
...
echo "78/78 sector-peer-promise-check (K-DM: sektor akrani vaadi)..."
