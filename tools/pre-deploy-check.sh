...
# 2. Python compile
echo ""
echo "2/69 Python compile (app.py)..."
...
# tutulmalı.
echo ""
echo "3/69 KALICI_KURALLAR audit..."
...
# 4. format-lint
echo ""
echo "4/69 format-lint (CPO-1180 K6)..."
...
echo ""
echo "5/69 CSS token guard (CPO-1349)..."
...
echo "6/69 style-guard (T1.7: sablon ici var()/ham hex/yerel :root/bos catch ratchet)..."
...
echo ""
echo "7/69 lint_scope ratchet (T9.4: sablon sayisi daralma dedektoru)..."
...
echo ""
echo "8/69 node-syntax-check (DEV2-T-MOBOVF-1: sablon-ici JS sozdizimi)..."
...
echo ""
echo "9/69 state-order-check (K-G: durum kurali varyantin ALTINDA olmali)..."
...
echo ""
echo "10/69 contrast-check (K-I: ayni kuralda bg+fg WCAG kontrasti)..."
...
echo ""
echo "11/69 cachebust-check (K-J: ?v= referansi diskteki hash ile ayni mi)..."
...
echo ""
echo "12/69 legal-text-sync-check (K-H: hukuki metin <-> 'Son guncelleme' senkronu)..."
...
echo ""
echo "13/69 da-override-check (K-K: .da-* bildirimini kisayolla silme)..."
...
echo ""
echo "14/69 canon-conflict-check (K-AP: kanonik kaynak <-> sayfa-yerel kopya)..."
...
echo ""
echo "15/69 glyph-canon-check (K-AQ: rozet glifi <-> kanonik anlam)..."
...
echo ""
echo "16/69 threshold-sync-check (K-AR: bant esigi kopyalari <-> business_rules)..."
...
echo ""
echo "17/69 nav-label-canon-check (K-AS: gezinme etiketi kanonu)..."
...
echo ""
echo "18/69 tooltip-canon-check (K-AT: ipucu mekanizmasi kanonu)..."
...
echo ""
echo "19/69 sort-canon-check (K-AV: siralama kontrolu durum kanonu)..."
...
echo ""
echo "20/69 newtab-canon-check (K-AW: yeni sekme uyarisi kanonu)..."
...
echo ""
echo "21/69 autorefresh-canon-check (K-AX: periyodik yenileme odak kanonu)..."
...
echo "22/69 sticky-anchor-check (K-AY: yapiskan capa kanonu)..."
...
echo "23/69 safearea-layer-check (K-AZ: guvenli alan kapsam kanonu)..."
...
echo "24/69 tr-decimal-input-check (K-BA: TR ondalik girdi kanonu)..."
...
echo "25/69 tr-calendar-day-check (K-BD: TR takvim gunu kanonu)..."
...
echo "26/69 global-collision-check (K-BE: kuresel ad cakismasi)..."
...
echo "27/69 js-palette-check (K-BG: JS paleti <-> tokens.css, RATCHET)..."
...
echo "28/69 copy-promise-check (K-BH: \"kopyala\" vaadi = kopyalanan sey)..."
...
echo "29/69 merge-report-check (K-BI: birlestirme sayimi = gercekten eklenen)..."
...
echo "30/69 position-validation-check (K-BJ: pozisyon sayisal dogrulamasi tek kanon)..."
...
echo "31/69 api-data-loading-guard (K-BK: bos liste != veri yok)..."
...
echo "32/69 local-data-guard (K-BM: bozuk yerel kayit sessizce silinmemeli)..."
...
echo "33/69 eod-freshness-claim-check (K-BN: gosterilen fiyatin tazelik vaadi)..."
...
echo "34/69 volume-axis-color-check (K-BO: hacim ekseni yon rengiyle boyanamaz)..."
...
echo "35/69 direction-zero-check (K-BP+K-CC: 0 bir yonle ayni kefeye konamaz)..."
...
echo "36/69 stop-level-canon-check (K-BQ: 'Stop' yon iddiasidir)..."
...
echo "37/69 indicator-precision-check (K-BR: ADX/RSI 1 ondalik kanonu)..."
...
echo "38/69 indicator-panel-canon-check (K-BS: rozet ve teknik satir tek kaynak)..."
...
echo "39/69 innerhtml-sibling-check (K-BT: innerHTML kardes bileseni yok etmemeli)..."
...
echo "40/69 label-derived-number-check (K-BU: sayi etiketten ayristirilmaz)..."
...
echo "41/69 money-scale-canon-check (K-BV: para olcegi kisaltmasi tek kanon)..."
...
echo "42/69 inline-style-hex-check (K-BW: satir-ici renk token'dan gelir)..."
...
echo "43/69 fundamental-card-band-check (K-BX: temel kart birim/band)..."
...
echo "44/69 template-color-channel-check (K-BY: renk kanaldan bagimsiz token'dan gelir)..."
...
echo "45/69 provenance-badge-check (K-BZ: koken rozeti <-> govdenin gercek kaynagi)..."
...
echo "46/69 rr-canon-check (K-CA: cubuk da sayi da rr_signal'den)..."
...
echo "47/69 window-claim-check (K-CB: pencere beyani <-> cizilen veri)..."
...
echo "48/69 export-parity-check (K-CC: disa aktarim <-> ekran ayni sayi/ad)..."
...
echo "49/69 relative-time-anchor-check (K-CD: goreli zaman <-> okuyucunun takvimi)..."
...
echo "50/69 intraday-claim-check (K-CE: EOD-only mimaride gun-ici/canli iddia)..."
...
echo "51/69 blog-claim-check (K-CF: blog govdesi icerik-dogruluk kanonu)..."
...
echo "52/69 app-published-text-check (K-CG: app.py yayimlanan metin kanonu)..."
...
echo "53/69 static-js-text-check (K-CH: paylasilan JS yayimlanan metin kanonu)..."
...
echo "54/69 manifest-text-check (K-CI: PWA manifest yayimlanan metin + URL sozu)..."
...
echo "55/69 crawler-channel-check (K-CJ: robots/llms/humans/security + JSON-LD)..."
...
#      duzeltildi.
echo "56/69 sitemap-claim-check (K-CK: changefreq <-> lastmod <-> EOD mimarisi)..."
...
echo "57/69 head-meta-check (K-CL: <head> meta kanali + og gorseli)..."
...
echo "58/69 glossary-where-check (K-CM: sozlugun \"nerede gorulur\" vaadi)..."
...
echo "59/69 score-claim-check (K-CN: Teknik Güç Skoru bilesen iddiasi)..."
...
echo "60/69 delivery-timing-claim-check (K-CO: teslimat anindaligi vaadi)..."
...
echo "61/69 tab-visibility-canon-check (K-CP: sekme gorunurlugu tek kanon)..."
...
echo "62/69 url-state-param-check (K-CQ: URL durum parametresi korunumu)..."
...
echo "63/69 form-url-state-check (K-CS: form durumu <-> adres cubugu tur-gidisi)..."
...
echo "64/69 url-state-owner-check (K-CT: adres cubugunun tek sahibi)..."
...
echo "65/69 client-server-state-owner-check (K-CU: sunucu durumunun istemcideki sahibi)..."
...
echo "66/69 privacy-claim-flow-check (K-CV: gizlilik beyani <-> veri akisi)..."
...
echo "67/69 score-band-canon-check (K-CW: skor bandinin renk+ad kanonu)..."
...
echo "68/69 dynamic-tip-canon-check (K-CX: durum degisince data-tip de guncellenmeli)..."
...
echo "69/69 canon-css-dup-check (K-CY: kanonik CSS'i JS enjeksiyonu ezemez)..."
